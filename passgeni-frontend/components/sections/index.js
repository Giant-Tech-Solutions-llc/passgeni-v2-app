...
const cls=(i)=>i===0?"b2":i===1?"b5 bc-feat":"b5";
...
{PRICING.plans.map((plan,i)=>(<div key={plan.name} className={`bc ${cls(i)}`} style={{display:"flex",flexDirection:"column"}}>
<div className="bc-line"/>
...
<a href={plan.ctaHref} className={plan.featured?"btn-primary":"btn-ghost"} style={{width:"100%",justifyContent:"center",marginBottom:24,display:"flex",fontSize:13,padding:"13px 20px",boxSizing:"border-box"}}>{plan.cta}</a>
...
{doubled.map((t,i)=>(<article key={i} className="bc" style={{width:300,flexShrink:0,padding:"22px 24px"}} onMouseEnter={e=>e.currentTarget.style.borderColor="rgba(200,255,0,.2)"} onMouseLeave={e=>e.currentTarget.style.borderColor="var(--border)"}>
...
<div><div style={{fontFamily:"var(--font-heading)",fontWeight:700,fontSize:13,color:"var(--text)"}}>{t.name}</div>
...
{slug:"announcing-passgeni-v2",title:"PassGeni V2: DNA Score, Compliance Presets, and 6 New Tools",excerpt:"Everything that shipped in V2 — the DNA Score, six new free tools, HIPAA and PCI-DSS presets.",category:"Product",color:"#81c784",readTime:4,date:"Jan 20, 2025"},
];
return(<section style={{padding:"0 var(--pad) var(--section)",maxWidth:1200,margin:"0 auto"}}>
