...
return(<section style={{borderTop:"1px solid rgba(200,255,0,0.08)",borderBottom:"1px solid rgba(200,255,0,0.08)"}}>
...
<div style={{fontFamily:"var(--font-mono)",fontSize:11,color:"var(--muted)",marginTop:8,letterSpacing:".1em",textTransform:"uppercase"}}>{label}</div>
</div>))}
</div>
...
<div style={{display:"flex",width:"max-content",animation:"ticker 50s linear infinite",gap:48,alignItems:"center",padding:"0 24px"}}
onMouseEnter={e=>e.currentTarget.style.animationPlayState="paused"}
onMouseLeave={e=>e.currentTarget.style.animationPlayState="running"}>
{[...LOGOS,...LOGOS].map((name,i)=>(<span key={i}style={{fontFamily:"var(--font-heading)",fontSize:16,fontWeight:800,color:"rgba(255,255,255,.15)",letterSpacing:"-.02em",whiteSpace:"nowrap",transition:"color .2s",cursor:"default"}}
onMouseEnter={e=>e.currentTarget.style.color="rgba(255,255,255,.35)"}
onMouseLeave={e=>e.currentTarget.style.color="rgba(255,255,255,.15)"}>{name}</span>))}
</div></div></div></section>);}
...
</div></section>);}import{useState}from"react";
import{STATS,HOW_IT_WORKS,FEATURES,TOOLS_PREVIEW,PRICING,TESTIMONIALS,FAQ,WAITLIST}from"../../content/copy.js";
import{Headline}from"../ui/index.js";
...
<div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:10,alignItems:"stretch"}}>
{FEATURES.items.slice(0,6).map((item,i)=>(<article key={item.title}className="bc bc-a"style={{minHeight:220}}>
<div className="bc-line"/>
<div style={{fontSize:28,marginBottom:16,color:"var(--accent)",lineHeight:1}}>{item.icon}</div>
...
<div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:10,alignItems:"stretch"}}>
{TOOLS_PREVIEW.items.slice(0,6).map((item,i)=>(<a key={item.title}href={item.href}className="bc bc-a"style={{textDecoration:"none",minHeight:220}}>
<div className="bc-line"/>
<div style={{fontSize:30,marginBottom:16,lineHeight:1}}>{item.icon}</div>
...
<div className="eyebrow"style={{justifyContent:"center"}}>{PRICING.eyebrow}</div>
<h2 style={{fontFamily:"var(--font-heading)",fontWeight:800,fontSize:"clamp(28px,4vw,52px)",color:"var(--text)",letterSpacing:"-.03em",marginBottom:14,whiteSpace:"pre-line"}}>{PRICING.headline}</h2>
<p style={{fontFamily:"var(--font-body)",fontSize:16,color:"var(--muted)",maxWidth:440,margin:"0 auto"}}>{PRICING.subheadline}</p>
</div>
...
<div style={{fontFamily:"var(--font-mono)",fontSize:10,color:plan.color||"var(--accent)",letterSpacing:".14em",textTransform:"uppercase",marginBottom:12}}>{plan.name}</div>
<div style={{display:"flex",alignItems:"baseline",gap:4,marginBottom:10}}>
<span style={{fontFamily:"var(--font-heading)",fontWeight:800,fontSize:56,color:"var(--text)",letterSpacing:"-.04em",lineHeight:1}}>{plan.price}</span>
{plan.period&&<span style={{fontFamily:"var(--font-body)",fontSize:15,color:"var(--muted)"}}>{plan.period}</span>}
</div>
<p style={{fontFamily:"var(--font-body)",fontSize:15,color:"var(--muted)",lineHeight:1.6}}>{plan.tagline}</p>
</div>
...
{plan.features.map((f,fi)=>(<div key={fi}style={{display:"flex",gap:10,alignItems:"flex-start",marginBottom:10}}>
<span style={{color:"var(--accent)",fontSize:12,marginTop:2,flexShrink:0}}>✓</span>
<span style={{fontFamily:"var(--font-body)",fontSize:14,color:"var(--text)",lineHeight:1.5}}>{f}</span>
</div>))}
{plan.missing?.slice(0,2).map((f,fi)=>(<div key={fi}style={{display:"flex",gap:10,alignItems:"flex-start",marginBottom:10,opacity:.3}}>
<span style={{fontSize:12,color:"var(--muted)",marginTop:2,flexShrink:0}}>—</span>
<span style={{fontFamily:"var(--font-body)",fontSize:14,color:"var(--muted)",lineHeight:1.5}}>{f}</span>
</div>))}
</div></div>))}
</div>
<p style={{textAlign:"center",fontFamily:"var(--font-mono)",fontSize:11,color:"var(--muted-2)",marginTop:28,letterSpacing:".08em"}}>{PRICING.footer}</p>
</section>);}
export function TestimonialsSection(){
const items=TESTIMONIALS.items;
const mid=Math.ceil(items.length/2);
...
{rows.map((row,ri)=>{
const doubled=[...row,...row];const dur=ri===0?90:110;
return(<div key={ri}style={{marginBottom:12,position:"relative"}}>
<div style={{position:"absolute",left:0,top:0,bottom:0,width:120,background:"linear-gradient(to right,var(--bg),transparent)",zIndex:2,pointerEvents:"none"}}/>
<div style={{position:"absolute",right:0,top:0,bottom:0,width:120,background:"linear-gradient(to left,var(--bg),transparent)",zIndex:2,pointerEvents:"none"}}/>
<div style={{display:"flex",gap:12,width:"max-content",animation:"scrollLeft "+dur+"s linear infinite",animationDirection:ri===1?"reverse":"normal"}}
onMouseEnter={e=>e.currentTarget.style.animationPlayState="paused"}
onMouseLeave={e=>e.currentTarget.style.animationPlayState="running"}>
{doubled.map((t,i)=>(<article key={i}className="bc"style={{width:320,flexShrink:0,padding:"26px 28px",minHeight:190}}
onMouseEnter={e=>e.currentTarget.style.borderColor="rgba(200,255,0,.25)"}
...
<div>
<div style={{fontFamily:"var(--font-heading)",fontWeight:700,fontSize:15,color:"var(--text)"}}>{t.name}</div>
<div style={{fontFamily:"var(--font-mono)",fontSize:11,color:"var(--muted-2)",marginTop:3,letterSpacing:".05em"}}>{t.role}</div>
</div></article>))}
</div></div>);})}
</section>);}
export function FAQSection(){
...
<div style={{display:"flex",justifyContent:"space-between",alignItems:"center",gap:24}}>
<div style={{display:"flex",gap:18,alignItems:"center"}}>
<span style={{fontFamily:"var(--font-mono)",fontSize:11,color:"rgba(200,255,0,.45)",letterSpacing:".1em",flexShrink:0,width:26}}>{String(i+1).padStart(2,"0")}</span>
<h3 style={{fontFamily:"var(--font-heading)",fontSize:17,fontWeight:700,color:open===i?"var(--accent)":"var(--text)",margin:0,lineHeight:1.4}}>{item.question}</h3>
</div>
<span style={{color:"var(--accent)",fontSize:22,flexShrink:0,transition:"transform .2s",transform:open===i?"rotate(45deg)":"none",display:"block"}}>+</span>
</div>
...
<h2 style={{fontFamily:"var(--font-heading)",fontWeight:800,fontSize:"clamp(24px,3.5vw,44px)",color:"var(--text)",letterSpacing:"-.03em"}}>Latest posts.</h2>
</div>
<div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:10,alignItems:"stretch"}}>
...
<span style={{fontFamily:"var(--font-mono)",fontSize:10,color:"var(--muted-2)"}}>{p.readTime} min</span>
</div>
...
<a href="/blog"style={{fontFamily:"var(--font-mono)",fontSize:12,color:"var(--accent)",letterSpacing:".08em",display:"inline-flex",alignItems:"center",gap:8,padding:"12px 28px",border:"1px solid rgba(200,255,0,.2)",borderRadius:8,transition:"border-color .2s,background .2s"}}
...
<div style={{fontFamily:"var(--font-mono)",fontSize:10,color:"var(--accent)",letterSpacing:".2em",textTransform:"uppercase",marginBottom:20,display:"flex",alignItems:"center",justifyContent:"center",gap:10}}>
<span className="live-dot"/>{WAITLIST.eyebrow}<span className="live-dot"/></div>
